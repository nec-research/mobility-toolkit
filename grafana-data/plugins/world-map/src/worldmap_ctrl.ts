...
export default class WorldmapCtrl extends MetricsPanelCtrl {
  ...
  refreshMetric() {
    this.panel.scopedVars = { "__zoom": { value: this.map.map._zoom }, "__metric": { value: this.panel.geometryMetric } };
    this.refresh();
  }
  ...
  onDataReceived(dataList) {
    ...
    try {
      ...
      } else if (this.panel.locationData === "geometry json") {
        this.dataFormatter.setGeometryJson(dataList, data);
      } else {
  ...
  setZoom() {
    this.map.setZoom(this.panel.initialZoom || 0);
  }
  ...
  link(scope, elem, attrs, ctrl) {
    ...
    function render() {
      ...
      if (ctrl.panel.locationData === "geometry json") {
        ctrl.map.drawGeometry();
      } else {
