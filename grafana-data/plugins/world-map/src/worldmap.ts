...
export default class WorldMap {
  ...
  createMap() {
    ...
    this.map = L.map(this.mapContainer, {
      ...
      preferCanvas: true,
      center: mapCenter,
      zoom: parseInt(this.ctrl.panel.initialZoom, 10) || 0,
    });
    ...
    // Zoom level event handling
    var map_obj = this;
    this.map.on('zoomend', function () {
      map_obj.RefreshPanelOnScopedVarUpdate(map_obj);
    });
  }
  RefreshPanelOnScopedVarUpdate(map_obj) {
    map_obj.ctrl.panel.scopedVars = { "__metric": { value: map_obj.ctrl.panel.geometryMetric }, "__zoom": { value: map_obj.map.getZoom() } };
    map_obj.ctrl.panel.refresh();
  }
  createLegend() {
  ...
  getCircleValuesArray(values) {
    var metricArr: any[] = [];
    for (var index in values) {
      var datapoints = values[index].datapoints;
      var targets = values[index].target.split("&&");
      var metricValue = -1;
      if (targets[1] != "" && datapoints.length == 1) {
        metricValue = parseInt(targets[1]);
        metricArr.push(metricValue);
      }
    }
    return metricArr;
  }
  ...
  drawGeometry() {
    this.clearGeometry();
    var values = this.ctrl.data.dataValues;
    const geometryArray: any[] = [];
    var lowestValue = 1;
    var highestValue = 1;
    var metricArr: any[] = [];
    metricArr = this.getCircleValuesArray(values);
    for (var index in values) {
      var datapoints = values[index].datapoints;
      var targets = values[index].target.split("&&");
      var metricValue = -1;
      ...
      var value = this.getColor(metricValue);
      ...
      if (datapoints.length == 1) {
        if (metricArr.length > 0) {
          lowestValue = Math.min(...metricArr);
          highestValue = Math.max(...metricArr);
        }

        geometryObj = (<any>window).L.circleMarker(datapoints[0], {
          radius: this.calculateCircleSize(metricValue, lowestValue, highestValue),
          color: value,
          fillColor: value,
          fillOpacity: 0.5,
        }).addTo(this.map);
        ...
        geometryObj = (<any>window).L.polygon(datapoints, {})
          .setStyle(myStyle).addTo(this.map);

      }
      this.createGeometryPopup(geometryObj,targets);

      this.addMouserHoverEvent(geometryObj);
      
      this.addMouseOutEvent(geometryObj);
      
      geometryArray.push(geometryObj);
    }
    this.geometryLayer = (<any>window).L.layerGroup(geometryArray).addTo(this.map);
    this.geometry = geometryArray;

    return geometryArray;
  }

  createGeometryPopup(geometryObj,targets){
    geometryObj.bindPopup(targets[0], {
  ...
  addMouserHoverEvent(geometryObj){
    geometryObj.on('mouseover', function onMouseOver(evt) {
  ...
  addMouseOutEvent(geometryObj){
    if (!this.ctrl.panel.stickyLabels) {
      geometryObj.on('mouseout', function onMouseOut() {
        geometryObj.closePopup();
      });
    }
  }
  ...
  calculateCircleSize(dataPointValue: any, lowestValue: any, highestValue: any) {
    const circleMinSize = parseInt(this.ctrl.panel.circleMinSize, 10) || 2;
    const circleMaxSize = parseInt(this.ctrl.panel.circleMaxSize, 10) || 30;
    ...
    const dataFactor = (dataPointValue - lowestValue) / valueRange;
    const circleSizeRange = circleMaxSize - circleMinSize;
